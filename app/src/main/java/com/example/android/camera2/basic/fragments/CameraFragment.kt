...
class CameraFragment : Fragment() {
                    ...
                    lifecycleScope.launch(Dispatchers.Main) {
                        navController.navigate(
                            CameraFragmentDirections
                                ...
                                .setOrientation(androidx.exifinterface.media.ExifInterface.ORIENTATION_NORMAL)
                                .setDepth(
    ...
    private suspend fun takePhoto():
        ...
        imageReader.setOnImageAvailableListener({ reader ->
            ...
            Log.d(TAG, "队列中可用的图像: ${image.timestamp}")
    ...
    private suspend fun saveResult(result: CombinedCaptureResult): Bitmap =
            ...
            when (result.format) {
                ...
                ImageFormat.JPEG, ImageFormat.DEPTH_JPEG -> {
                    val bitmapOptions = BitmapFactory.Options().apply {
                        ...
                        if (max(result.image.height, result.image.width) > ImageViewerFragment.DOWNSAMPLE_SIZE) {
                            val scaleFactorX = result.image.width / ImageViewerFragment.DOWNSAMPLE_SIZE + 1
                            val scaleFactorY = result.image.height / ImageViewerFragment.DOWNSAMPLE_SIZE + 1
                    ...
                    var bitmapImage = BitmapFactory.decodeByteArray(bytes, 0, bytes.size, bitmapOptions)
                    ...
                    Log.d(TAG, "saveResult: " + result.image.height + " " + result.image.width)
                    Log.d(TAG, "saveResult: " + bitmapImage.height + " " + bitmapImage.width)
                    try {
                        cont.resume(bitmapImage)
